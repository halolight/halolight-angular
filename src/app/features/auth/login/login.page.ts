        ...
        <ui-card class="border border-border/50 shadow-2xl backdrop-blur-xl bg-card/80 overflow-hidden">
          ...
          <ui-card-footer class="flex-col space-y-3 sm:space-y-4 px-4 sm:px-6 pb-5 sm:pb-8 pt-2">
            ...
            <p class="text-xs sm:text-sm text-muted-foreground text-center">
              还没有账户？
              <a routerLink="/auth/register" class="text-primary hover:text-primary/80 font-semibold transition-colors">
                立即注册
              </a>
            </p>
            <p class="text-xs text-muted-foreground text-center">
              阅读我们的
              <a routerLink="/terms" class="font-semibold text-foreground hover:text-primary transition-colors">服务条款</a>
              和
              <a routerLink="/privacy" class="font-semibold text-foreground hover:text-primary transition-colors">隐私政策</a>
              了解更多信息。
            </p>
            @if (showDemoHint) {
...
export class LoginPage {
  ...
  // 环境变量配置
  readonly showDemoHint = environment.showDemoHint;
